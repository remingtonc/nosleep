...
fn main() {
    ...
    info!("Sampling by sleeping thread every {:?}...", sleep_time);
    loop {
        ...
        if sample.as_secs() > (sleep_time.as_secs() as f64 * 1.5) as u64 {
            warn!("System appears to have slept! Expected ~{sleep_time:?}, calculated {sample:?}.", sleep_time=sleep_time, sample=sample);
            break;
        }
    }
}